...
% labeledImage = bwlabel(seg_im);
% stats = regionprops(labeledImage, 'Area');
% areas = [stats.Area];
% [~, idx] = max(areas);
% seg_im = labeledImage == idx;
...
whitePixels = find(seg_im == 1); 

% Iterate over the white pixels and set the pixels below them to white
for i = 1:numel(whitePixels)
    [row, col] = ind2sub(size(seg_im), whitePixels(i));
    seg_im(row+1:end, col) = 255; % Set pixels below to white
end
...
mu              = 0.1;
nbGVFiter       = 5; %5
sigma           = 3;            % Standard deviation of the gaussian kernel applied to the processed image
...
%      linspace(yleft, ycenter, (bound_right+step)/step /2), ...
%      linspace(ycenter, yright, (bound_right+2*step)/step /2 )

%lineX = lineX(bound_left/18:1:bound_right/18);
%lineY = lineY(bound_left/18:1:bound_right/18);


[Y,X] = GVFSnake(ori_im,px,py,lineX,lineY, alpha,beta,gamma,kappa,nbiter,stepIter,maxChange, fid);
